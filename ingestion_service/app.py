...
async def create_batch_tickets(request: BatchTicketRequest) -> BatchTicketResponse:
    ...
    try:
        ...
        # Оценка времени обработки с учетом параллельности Worker'а
        # Для малых пакетов: 2 секунды на тикет (консервативно)
        # Для больших пакетов: ~0.4 секунды на тикет + 5 секунд базовое время
        # (учитывается параллельная обработка Worker'ом)
        if queued <= 10:
            estimated_time = queued * 2000  # 2 сек на тикет для малых пакетов
        else:
            # Для больших пакетов учитываем параллельность обработки
            estimated_time = int(queued * 0.4 * 1000) + 5000  # ~0.4 сек на тикет + 5 сек базовое
